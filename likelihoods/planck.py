...
References
----------
Chen, Kumar & Ratra (2019), ApJ 887, 133, arXiv:1808.05724.
"""
...
from data.loader import load_planck
...
class PlanckLikelihood(BaseLikelihood):
    ...
    def model(
        ...
        D_M(z*) is obtained by integrating chi(z) = int dz/E(z)
        directly out to z* ~ 1090, rather than through the
        cosmology's cached low-z distance-integrator grid (which
        only spans z=0..5 by default, sized for the low-redshift
        CC/BAO/SN likelihoods). This keeps the low-z grid's
        resolution -- and the performance of every other
        likelihood -- unaffected by the CMB's much larger
        redshift range.
        """
        ...
        chi_star, _ = quad(
            lambda zz: 1.0 / recomb.E_recomb(zz),
            0.0,
            z_star,
            limit=200,
        )
