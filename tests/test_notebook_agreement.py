...
# ============================================================
#
# Everything above compares E(z). That leaves the half of the
# physics a modified-gravity model is actually distinguished by
# untested against the notebooks -- and it is the half where a sign
# can hide, because the two codebases disagree about the sign of
# the torsion scalar *on purpose*.
#
# GR-06 puts it at T = +6H^2, with TEGR at f = -T, and derives
# G_eff/G = **-1/f_T**. The `theory` package puts it at T = -6H^2
# and uses **+1/f_T**. Two sign conventions and two sign
# conventions' worth of minus signs, which cancel -- so the same
# physical model has to come out the same number, and would not if
# one of them were ever flipped alone.
#
# The same theory written in each convention:
#
#     CosmoFit   f(T_CF) = T_CF + A0 (-T_CF)^n,   T_CF = -6E^2
#     GR-06      f(T)    = -T   + A0 T^n,         T    = +6E^2
#
# and the second is TEGR at A0 = 0, which is the check that the
# translation itself is right before any number is compared.
#
# Regenerated by `mu06.wls` against GR-06's own `geffSubHorizon`.

#: (n, z, mu), from GR-06, at Omega_m = 0.3.
MU_REFERENCE = [
    (-0.5, 0.0, 1.21212121212121212121),
    (-0.5, 0.5, 1.09754428112826638687),
    (-0.5, 1.0, 1.03843463559335845028),
    (-0.5, 2.0, 1.00731594926099762198),
    (0.2, 0.0, 0.81081081081081081081),
    (0.2, 0.5, 0.87274272663580922632),
    (0.2, 1.0, 0.91740109750366681122),
    (0.2, 2.0, 0.96194244125016557378),
]
...
def test_mu_matches_the_notebook(n, z, expected):
    ...
    got = float(model.mu(1.0 / (1.0 + z)))

    assert got == pytest.approx(expected, rel=RTOL)


def test_the_notebook_agrees_about_where_the_model_is_sick():
    """
    Agreement on the healthy branch is worth more if the two also
    agree about the unhealthy one.

    At ``n = 0.7`` GR-06's own expression gives
    ``mu = -4.44, +25.6, +4.70, +2.38`` across ``z = 0, 0.5, 1, 2``:
    negative, then through the pole. `coupling_from_derivative`
    refuses exactly that, so the guard is declining something the
    notebook independently says is unphysical rather than an
    ...
    with pytest.raises(ValueError, match="repulsive gravity"):
        model.mu(np.array([1.0, 1.0 / 1.5, 0.5]))
